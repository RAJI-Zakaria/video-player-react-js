...
interface DisplayerProps {
  ...
  const Displayer: React.FC<DisplayerProps> = ({ connected, messages, handleTimeClick }) => {
    ...
    const renderMessageContent = (content: string) => {
      ...
      while ((match = timeRegex.exec(content)) !== null) {
        ...
        if (startIndex > lastIndex) {
          parts.push(<span key={lastIndex}>{content.substring(lastIndex, startIndex)}</span>);
        }
  
        // Add time part as a button
        parts.push(
          <button className="p-0 px-2 rounded" key={startIndex} onClick={() => handleTimeClick(time)}>
            {time}
          </button>
        );
            ...
            <ul className="list-group">
              {messages.map((message, index) => (
                <li className="list-group-item" key={index}>
                  <p><strong>{message.name}:</strong> {renderMessageContent(message.message)}</p>
