...
const ChatRoom: React.FC<{ handleTimeClick: (time: string) => void }> = ({ handleTimeClick }) => {
    ...
    const submitMessage = (messageString: string) => {
      const message: Message = { name: nameInput, message: messageString, when: Date.now() };
    ...
    return (
      <div className='chat-room-container'>
        ...
        <Displayer connected={connected} messages={messages} handleTimeClick={handleTimeClick} />
        <Composer connected={connected} nameInput={nameInput} messageInput={messageInput} setNameInput={setNameInput} setMessageInput={setMessageInput} submitMessage={submitMessage} />
      </div>
    );
  }
